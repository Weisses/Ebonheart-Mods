...
public class RenderGemstoneConverter extends TileEntitySpecialRenderer<TileEntityGemstoneConverter> {
	
	//private ModelGemstoneConverter model;
	
	public RenderGemstoneConverter() 
	{
		//this.model = new ModelGemstoneConverter();
	}
	
	@Override
	public void render(TileEntityGemstoneConverter te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
	{
		...
		&& te.cutTime >= 1)
		{
			...
			if (d <= 50)
			{
				//InitParticlesVGRender.generateRunicConverterParticles(te);
			}
		}
	...
	public boolean isGlobalRenderer(TileEntityGemstoneConverter te) 
	{
        return true;
    }
	...
	private void renderItemFuel(TileEntityGemstoneConverter te, double x, double y, double z, float partialTicks, int destroyStage)
    {
