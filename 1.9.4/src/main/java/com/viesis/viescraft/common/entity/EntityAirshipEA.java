...
public class EntityAirshipEA extends Entity {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipEA.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipEA.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipEA.class, DataSerializers.FLOAT);
    ...
    private EntityAirshipEA.Status status;
    private EntityAirshipEA.Status previousStatus;
    ...
    {
        if (entityIn instanceof EntityAirshipEA)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipEA.Status.UNDER_WATER && this.status != EntityAirshipEA.Status.UNDER_FLOWING_WATER)
        {
        ...
        LogHelper.info("Up pressed: " + this.upInputDown);
        
        if (//this.isPassenger(playerIn)
     ...
     */
    private EntityAirshipEA.Status getBoatStatus()
    {
        EntityAirshipEA.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipEA.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipEA.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipEA.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipEA.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipEA.Status EntityAirshipEA$status = EntityAirshipEA.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipEA.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipEA.Status.IN_AIR && this.status != EntityAirshipEA.Status.IN_AIR && this.status != EntityAirshipEA.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipEA.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipEA.Status.IN_WATER)
            {
                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirshipEA.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirshipEA.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipEA.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityAirshipEA.Status.ON_LAND)
            {
    ...
    private void controlBoat()
    {
        if (this.isBeingRidden())
        {
            ...
            this.motionY += (double)(MathHelper.cos(this.rotationYaw * 0.017453292F) * f1);
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipEA.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityAirshipEA.Status.ON_LAND)
                    {
    ...
    public void setBoatType(EntityAirshipEA.Type boatType)
    {
    ...
    @SideOnly(Side.CLIENT)
    public void updateInputs(boolean p_184442_1_, boolean p_184442_2_, boolean p_184442_3_, boolean p_184442_4_, boolean p_184442_5_)
    {
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipEA.Type byId(int id)
        {
        ...
        public static EntityAirshipEA.Type getTypeFromString(String nameIn)
        {
