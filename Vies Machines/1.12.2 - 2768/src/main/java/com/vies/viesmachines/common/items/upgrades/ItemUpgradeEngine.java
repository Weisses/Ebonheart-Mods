...
public class ItemUpgradeEngine extends Item {
	...
	public ItemUpgradeEngine(String unlocalizedNameIn, EnumRarity rarityIn, int upgradeTierIn) 
	{
		...
		this.procName = "UpgradeEngineProc";
	...
	public boolean onLeftClickEntity(ItemStack stack, EntityPlayer player, Entity entity)
    {
		if (entity instanceof EntityMachineBase)
        {
			...
			{
				if (((EntityMachineBase) entity).getTierEngine() < (this.upgradeTier)
				&& ((EntityMachineBase) entity).getTierEngine() == (this.upgradeTier - 1))
				{
					((EntityMachineBase) entity).setTierEngine(this.upgradeTier);
		...
		else if(entity instanceof EntityLiving)
        {
			if (entity.getRidingEntity() != null)
			{
				if (entity.getRidingEntity() instanceof EntityMachineBase)
				{
					...
					{
						if (((EntityMachineBase) entity.getRidingEntity()).getTierEngine() < (this.upgradeTier)
						&& ((EntityMachineBase) entity.getRidingEntity()).getTierEngine() == (this.upgradeTier - 1))
						{
							((EntityMachineBase) entity.getRidingEntity()).setTierEngine(this.upgradeTier);
