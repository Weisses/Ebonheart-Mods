...
public class ContainerMachineTransmogrifier extends ContainerTileEntityVM {
	
	private final World world;
    private final TileEntityMachineTransmogrifier appliance;
    ...
    public ContainerMachineTransmogrifier(InventoryPlayer playerInventory, World worldIn, TileEntityMachineTransmogrifier applianceIn)
    {
    	this.world = worldIn;
    ...
    public boolean canInteractWith(EntityPlayer playerIn)
    {
        ...
        return this.world.getBlockState(this.appliance.getPos()).getBlock() != BlocksVM.MACHINE_TRANSMOGRIFIER ? false : playerIn.getDistanceSq((double)this.appliance.getPos().getX() + 0.5D, (double)this.appliance.getPos().getY() + 0.5D, (double)this.appliance.getPos().getZ() + 0.5D) <= 64.0D;
    }
