...
public class CopyOfEntityAirshipBase extends EntityVC {
	...
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(CopyOfEntityAirshipBase.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(CopyOfEntityAirshipBase.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(CopyOfEntityAirshipBase.class, DataSerializers.FLOAT);
	...
	public CopyOfEntityAirshipBase.Status status;
    public CopyOfEntityAirshipBase.Status previousStatus;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != CopyOfEntityAirshipBase.Status.UNDER_WATER && this.status != CopyOfEntityAirshipBase.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private CopyOfEntityAirshipBase.Status getAirshipStatus()
    {
        CopyOfEntityAirshipBase.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return CopyOfEntityAirshipBase.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return CopyOfEntityAirshipBase.Status.ON_LAND;
            }
            else
            {
                return CopyOfEntityAirshipBase.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private CopyOfEntityAirshipBase.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                CopyOfEntityAirshipBase.Status EntityAirshipEA$status = CopyOfEntityAirshipBase.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? CopyOfEntityAirshipBase.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == CopyOfEntityAirshipBase.Status.IN_AIR && this.status != CopyOfEntityAirshipBase.Status.IN_AIR && this.status != CopyOfEntityAirshipBase.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = CopyOfEntityAirshipBase.Status.IN_WATER;
        }
        else
        {
            if (this.status == CopyOfEntityAirshipBase.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == CopyOfEntityAirshipBase.Status.UNDER_FLOWING_WATER 
        	  || this.status == CopyOfEntityAirshipBase.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == CopyOfEntityAirshipBase.Status.IN_AIR
            	  || this.status == CopyOfEntityAirshipBase.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    public void setBoatType(CopyOfEntityAirshipBase.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public CopyOfEntityAirshipBase.Type getBoatType()
    {
        return CopyOfEntityAirshipBase.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static CopyOfEntityAirshipBase.Type byId(int id)
        {
        ...
        public static CopyOfEntityAirshipBase.Type getTypeFromString(String nameIn)
        {
