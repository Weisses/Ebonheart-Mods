...
public class EntityAirshipOrange extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipOrange.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipOrange.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipOrange.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipOrange.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipOrange.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipOrange.Status status;
    public EntityAirshipOrange.Status previousStatus;
	...
	public EntityAirshipOrange(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipOrange(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipOrange.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_orange;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipOrange.Status.UNDER_WATER && this.status != EntityAirshipOrange.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipOrange.Status getAirshipStatus()
    {
        EntityAirshipOrange.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipOrange.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipOrange.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipOrange.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipOrange.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipOrange.Status EntityAirshipEA$status = EntityAirshipOrange.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipOrange.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipOrange.Status.IN_AIR && this.status != EntityAirshipOrange.Status.IN_AIR && this.status != EntityAirshipOrange.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipOrange.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipOrange.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipOrange.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipOrange.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipOrange.Status.IN_AIR
            	  || this.status == EntityAirshipOrange.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipOrange.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipOrange.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipOrange.Type getBoatType()
    {
        return EntityAirshipOrange.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipOrange.Type byId(int id)
        {
        ...
        public static EntityAirshipOrange.Type getTypeFromString(String nameIn)
        {
