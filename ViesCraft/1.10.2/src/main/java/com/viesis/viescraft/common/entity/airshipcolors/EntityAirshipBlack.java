...
public class EntityAirshipBlack extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipBlack.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipBlack.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipBlack.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipBlack.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipBlack.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipBlack.Status status;
    public EntityAirshipBlack.Status previousStatus;
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipBlack.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipBlack.Status.UNDER_WATER && this.status != EntityAirshipBlack.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipBlack.Status getAirshipStatus()
    {
        EntityAirshipBlack.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipBlack.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipBlack.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipBlack.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipBlack.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipBlack.Status EntityAirshipEA$status = EntityAirshipBlack.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipBlack.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipBlack.Status.IN_AIR && this.status != EntityAirshipBlack.Status.IN_AIR && this.status != EntityAirshipBlack.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipBlack.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipBlack.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipBlack.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipBlack.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipBlack.Status.IN_AIR
            	  || this.status == EntityAirshipBlack.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipBlack.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipBlack.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipBlack.Type getBoatType()
    {
        return EntityAirshipBlack.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipBlack.Type byId(int id)
        {
        ...
        public static EntityAirshipBlack.Type getTypeFromString(String nameIn)
        {
