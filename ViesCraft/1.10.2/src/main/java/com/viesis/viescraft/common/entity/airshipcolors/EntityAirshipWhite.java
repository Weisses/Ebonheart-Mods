...
public class EntityAirshipWhite extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipWhite.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipWhite.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipWhite.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipWhite.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipWhite.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipWhite.Status status;
    public EntityAirshipWhite.Status previousStatus;
	...
	public EntityAirshipWhite(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipWhite(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipWhite.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_white;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipWhite.Status.UNDER_WATER && this.status != EntityAirshipWhite.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipWhite.Status getAirshipStatus()
    {
        EntityAirshipWhite.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipWhite.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipWhite.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipWhite.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipWhite.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipWhite.Status EntityAirshipEA$status = EntityAirshipWhite.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipWhite.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipWhite.Status.IN_AIR && this.status != EntityAirshipWhite.Status.IN_AIR && this.status != EntityAirshipWhite.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipWhite.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipWhite.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipWhite.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipWhite.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipWhite.Status.IN_AIR
            	  || this.status == EntityAirshipWhite.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipWhite.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipWhite.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipWhite.Type getBoatType()
    {
        return EntityAirshipWhite.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipWhite.Type byId(int id)
        {
        ...
        public static EntityAirshipWhite.Type getTypeFromString(String nameIn)
        {
