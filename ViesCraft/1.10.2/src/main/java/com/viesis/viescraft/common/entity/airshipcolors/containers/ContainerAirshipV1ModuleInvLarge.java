...
public class ContainerAirshipV1ModuleInvLarge extends Container {
	 ...
	 */
	public ContainerAirshipV1ModuleInvLarge(IInventory playerInv, EntityAirshipV1Core airship) 
	{
		...
		for (int y = 0; y < 3; ++y) 
		{
			for (int x = 0; x < 3; ++x) 
			{
				this.addSlotToContainer(new InventorySlotVC(airship, (x + y * 3) + 2, 8 + x * 18, 17 + y * 18));
			}
		}
		for (int y = 0; y < 3; ++y) 
		{
			for (int x = 0; x < 3; ++x) 
			{
				this.addSlotToContainer(new InventorySlotVC(airship, ((x + y * 3) + 2) + 9, 62 + x * 18, 17 + y * 18));
			}
		}
