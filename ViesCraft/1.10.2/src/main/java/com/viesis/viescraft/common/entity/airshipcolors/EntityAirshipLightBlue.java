...
public class EntityAirshipLightBlue extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipLightBlue.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipLightBlue.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipLightBlue.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipLightBlue.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipLightBlue.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipLightBlue.Status status;
    public EntityAirshipLightBlue.Status previousStatus;
	...
	public EntityAirshipLightBlue(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipLightBlue(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipLightBlue.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_lightblue;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipLightBlue.Status.UNDER_WATER && this.status != EntityAirshipLightBlue.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipLightBlue.Status getAirshipStatus()
    {
        EntityAirshipLightBlue.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipLightBlue.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipLightBlue.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipLightBlue.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipLightBlue.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipLightBlue.Status EntityAirshipEA$status = EntityAirshipLightBlue.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipLightBlue.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipLightBlue.Status.IN_AIR && this.status != EntityAirshipLightBlue.Status.IN_AIR && this.status != EntityAirshipLightBlue.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipLightBlue.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipLightBlue.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipLightBlue.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipLightBlue.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipLightBlue.Status.IN_AIR
            	  || this.status == EntityAirshipLightBlue.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipLightBlue.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipLightBlue.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipLightBlue.Type getBoatType()
    {
        return EntityAirshipLightBlue.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipLightBlue.Type byId(int id)
        {
        ...
        public static EntityAirshipLightBlue.Type getTypeFromString(String nameIn)
        {
