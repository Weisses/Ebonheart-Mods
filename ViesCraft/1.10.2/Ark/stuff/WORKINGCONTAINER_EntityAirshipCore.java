...
public class WORKINGCONTAINER_EntityAirshipCore extends EntityVC implements IInventory {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(WORKINGCONTAINER_EntityAirshipCore.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(WORKINGCONTAINER_EntityAirshipCore.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(WORKINGCONTAINER_EntityAirshipCore.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(WORKINGCONTAINER_EntityAirshipCore.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(WORKINGCONTAINER_EntityAirshipCore.class, DataSerializers.BOOLEAN);
    private int fuel;
    private ItemStack[] inventory;
	private String customName;
	
	public WORKINGCONTAINER_EntityAirshipCore.Status status;
    public WORKINGCONTAINER_EntityAirshipCore.Status previousStatus;
	...
	public WORKINGCONTAINER_EntityAirshipCore(World worldIn)
    {
        super(worldIn);
    ...
    public WORKINGCONTAINER_EntityAirshipCore(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(WORKINGCONTAINER_EntityAirshipCore.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_WATER && this.status != WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private WORKINGCONTAINER_EntityAirshipCore.Status getAirshipStatus()
    {
        WORKINGCONTAINER_EntityAirshipCore.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return WORKINGCONTAINER_EntityAirshipCore.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return WORKINGCONTAINER_EntityAirshipCore.Status.ON_LAND;
            }
            else
            {
                return WORKINGCONTAINER_EntityAirshipCore.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private WORKINGCONTAINER_EntityAirshipCore.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                WORKINGCONTAINER_EntityAirshipCore.Status EntityAirshipEA$status = WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == WORKINGCONTAINER_EntityAirshipCore.Status.IN_AIR && this.status != WORKINGCONTAINER_EntityAirshipCore.Status.IN_AIR && this.status != WORKINGCONTAINER_EntityAirshipCore.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = WORKINGCONTAINER_EntityAirshipCore.Status.IN_WATER;
        }
        else
        {
            if (this.status == WORKINGCONTAINER_EntityAirshipCore.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_FLOWING_WATER 
        	  || this.status == WORKINGCONTAINER_EntityAirshipCore.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == WORKINGCONTAINER_EntityAirshipCore.Status.IN_AIR
            	  || this.status == WORKINGCONTAINER_EntityAirshipCore.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    {
    	player.openGui(Reference.MOD_ID, GuiHandler.ENTITYAIRSHIPVC_GUI, world, player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ());
    ...
    public void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(WORKINGCONTAINER_EntityAirshipCore.Type.getTypeFromString(compound.getString("Type")));
        }
    ...
    public void setBoatType(WORKINGCONTAINER_EntityAirshipCore.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public WORKINGCONTAINER_EntityAirshipCore.Type getBoatType()
    {
        return WORKINGCONTAINER_EntityAirshipCore.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static WORKINGCONTAINER_EntityAirshipCore.Type byId(int id)
        {
        ...
        public static WORKINGCONTAINER_EntityAirshipCore.Type getTypeFromString(String nameIn)
        {
	...
	{
		return (WORKINGCONTAINER_EntityAirshipCore)player.getRidingEntity() == this
