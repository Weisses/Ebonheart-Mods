...
public class EntityAirshipYellow extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipYellow.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipYellow.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipYellow.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipYellow.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipYellow.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipYellow.Status status;
    public EntityAirshipYellow.Status previousStatus;
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipYellow.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipYellow.Status.UNDER_WATER && this.status != EntityAirshipYellow.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipYellow.Status getAirshipStatus()
    {
        EntityAirshipYellow.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipYellow.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipYellow.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipYellow.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipYellow.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipYellow.Status EntityAirshipEA$status = EntityAirshipYellow.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipYellow.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipYellow.Status.IN_AIR && this.status != EntityAirshipYellow.Status.IN_AIR && this.status != EntityAirshipYellow.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipYellow.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipYellow.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipYellow.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipYellow.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipYellow.Status.IN_AIR
            	  || this.status == EntityAirshipYellow.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipYellow.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipYellow.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipYellow.Type getBoatType()
    {
        return EntityAirshipYellow.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipYellow.Type byId(int id)
        {
        ...
        public static EntityAirshipYellow.Type getTypeFromString(String nameIn)
        {
