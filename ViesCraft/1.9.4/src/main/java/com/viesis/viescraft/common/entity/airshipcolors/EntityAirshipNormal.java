...
public class EntityAirshipNormal extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipNormal.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipNormal.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipNormal.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipNormal.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipNormal.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipNormal.Status status;
    public EntityAirshipNormal.Status previousStatus;
	...
	public EntityAirshipNormal(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipNormal(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipNormal.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipNormal.Status.UNDER_WATER && this.status != EntityAirshipNormal.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipNormal.Status getAirshipStatus()
    {
        EntityAirshipNormal.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipNormal.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipNormal.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipNormal.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipNormal.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipNormal.Status EntityAirshipEA$status = EntityAirshipNormal.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipNormal.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipNormal.Status.IN_AIR && this.status != EntityAirshipNormal.Status.IN_AIR && this.status != EntityAirshipNormal.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipNormal.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipNormal.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipNormal.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipNormal.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipNormal.Status.IN_AIR
            	  || this.status == EntityAirshipNormal.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipNormal.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipNormal.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipNormal.Type getBoatType()
    {
        return EntityAirshipNormal.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipNormal.Type byId(int id)
        {
        ...
        public static EntityAirshipNormal.Type getTypeFromString(String nameIn)
        {
