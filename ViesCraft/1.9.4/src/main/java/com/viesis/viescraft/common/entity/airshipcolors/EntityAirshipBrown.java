...
public class EntityAirshipBrown extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipBrown.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipBrown.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipBrown.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipBrown.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipBrown.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipBrown.Status status;
    public EntityAirshipBrown.Status previousStatus;
	...
	public EntityAirshipBrown(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipBrown(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipBrown.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_brown;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipBrown.Status.UNDER_WATER && this.status != EntityAirshipBrown.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipBrown.Status getAirshipStatus()
    {
        EntityAirshipBrown.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipBrown.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipBrown.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipBrown.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipBrown.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipBrown.Status EntityAirshipEA$status = EntityAirshipBrown.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipBrown.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipBrown.Status.IN_AIR && this.status != EntityAirshipBrown.Status.IN_AIR && this.status != EntityAirshipBrown.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipBrown.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipBrown.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipBrown.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipBrown.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipBrown.Status.IN_AIR
            	  || this.status == EntityAirshipBrown.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipBrown.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipBrown.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipBrown.Type getBoatType()
    {
        return EntityAirshipBrown.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipBrown.Type byId(int id)
        {
        ...
        public static EntityAirshipBrown.Type getTypeFromString(String nameIn)
        {
