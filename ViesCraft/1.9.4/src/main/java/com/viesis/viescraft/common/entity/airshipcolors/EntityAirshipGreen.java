...
public class EntityAirshipGreen extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipGreen.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipGreen.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipGreen.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipGreen.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipGreen.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipGreen.Status status;
    public EntityAirshipGreen.Status previousStatus;
	...
	public EntityAirshipGreen(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipGreen(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipGreen.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_green;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipGreen.Status.UNDER_WATER && this.status != EntityAirshipGreen.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipGreen.Status getAirshipStatus()
    {
        EntityAirshipGreen.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipGreen.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipGreen.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipGreen.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipGreen.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipGreen.Status EntityAirshipEA$status = EntityAirshipGreen.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipGreen.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipGreen.Status.IN_AIR && this.status != EntityAirshipGreen.Status.IN_AIR && this.status != EntityAirshipGreen.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipGreen.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipGreen.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipGreen.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipGreen.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipGreen.Status.IN_AIR
            	  || this.status == EntityAirshipGreen.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipGreen.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipGreen.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipGreen.Type getBoatType()
    {
        return EntityAirshipGreen.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipGreen.Type byId(int id)
        {
        ...
        public static EntityAirshipGreen.Type getTypeFromString(String nameIn)
        {
