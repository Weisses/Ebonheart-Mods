...
public class EntityAirshipLightGray extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipLightGray.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipLightGray.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipLightGray.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipLightGray.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipLightGray.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipLightGray.Status status;
    public EntityAirshipLightGray.Status previousStatus;
	...
	public EntityAirshipLightGray(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipLightGray(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipLightGray.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_lightgray;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipLightGray.Status.UNDER_WATER && this.status != EntityAirshipLightGray.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipLightGray.Status getAirshipStatus()
    {
        EntityAirshipLightGray.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipLightGray.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipLightGray.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipLightGray.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipLightGray.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipLightGray.Status EntityAirshipEA$status = EntityAirshipLightGray.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipLightGray.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipLightGray.Status.IN_AIR && this.status != EntityAirshipLightGray.Status.IN_AIR && this.status != EntityAirshipLightGray.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipLightGray.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipLightGray.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipLightGray.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipLightGray.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipLightGray.Status.IN_AIR
            	  || this.status == EntityAirshipLightGray.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipLightGray.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipLightGray.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipLightGray.Type getBoatType()
    {
        return EntityAirshipLightGray.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipLightGray.Type byId(int id)
        {
        ...
        public static EntityAirshipLightGray.Type getTypeFromString(String nameIn)
        {
