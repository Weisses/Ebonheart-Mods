...
public class EntityAirshipPurple extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipPurple.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipPurple.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipPurple.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipPurple.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipPurple.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipPurple.Status status;
    public EntityAirshipPurple.Status previousStatus;
	...
	public EntityAirshipPurple(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipPurple(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipPurple.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_purple;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipPurple.Status.UNDER_WATER && this.status != EntityAirshipPurple.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipPurple.Status getAirshipStatus()
    {
        EntityAirshipPurple.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipPurple.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipPurple.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipPurple.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipPurple.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipPurple.Status EntityAirshipEA$status = EntityAirshipPurple.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipPurple.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipPurple.Status.IN_AIR && this.status != EntityAirshipPurple.Status.IN_AIR && this.status != EntityAirshipPurple.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipPurple.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipPurple.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipPurple.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipPurple.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipPurple.Status.IN_AIR
            	  || this.status == EntityAirshipPurple.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipPurple.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipPurple.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipPurple.Type getBoatType()
    {
        return EntityAirshipPurple.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipPurple.Type byId(int id)
        {
        ...
        public static EntityAirshipPurple.Type getTypeFromString(String nameIn)
        {
