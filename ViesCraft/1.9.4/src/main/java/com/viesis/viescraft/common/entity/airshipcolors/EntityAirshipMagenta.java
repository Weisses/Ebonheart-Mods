...
public class EntityAirshipMagenta extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipMagenta.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipMagenta.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipMagenta.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipMagenta.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipMagenta.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipMagenta.Status status;
    public EntityAirshipMagenta.Status previousStatus;
	...
	public EntityAirshipMagenta(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipMagenta(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipMagenta.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipMagenta.Status.UNDER_WATER && this.status != EntityAirshipMagenta.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipMagenta.Status getAirshipStatus()
    {
        EntityAirshipMagenta.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipMagenta.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipMagenta.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipMagenta.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipMagenta.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipMagenta.Status EntityAirshipEA$status = EntityAirshipMagenta.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipMagenta.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipMagenta.Status.IN_AIR && this.status != EntityAirshipMagenta.Status.IN_AIR && this.status != EntityAirshipMagenta.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipMagenta.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipMagenta.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipMagenta.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipMagenta.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipMagenta.Status.IN_AIR
            	  || this.status == EntityAirshipMagenta.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipMagenta.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipMagenta.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipMagenta.Type getBoatType()
    {
        return EntityAirshipMagenta.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipMagenta.Type byId(int id)
        {
        ...
        public static EntityAirshipMagenta.Type getTypeFromString(String nameIn)
        {
