...
public class ItemUpgradeFrame extends Item {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
		...
		{
			tooltip.add(References.localNameVC("vc.item.tt.upgradeframe.0.1"));
			tooltip.add(References.localNameVC("vc.item.tt.upgradeframe.0.2"));
			tooltip.add("");
			tooltip.add(References.localNameVC("vc.item.tt.upgradeframe.0.3"));
		}
		else if(gameSettingsIn.isKeyDown(gameSettingsIn.keyBindSneak))
		{
			if(stack.getMetadata() == 1)
			{
				...
				tooltip.add(stringColorText + References.localNameVC("vc.item.tt.upgradeframe.#.1"));
				tooltip.add("");
				...
				//New frame skins each Tier.
				tooltip.add(TextFormatting.GREEN + "- " + TextFormatting.BLUE + References.localNameVC("vc.item.tt.upgradeframe.#.2"));
			}
			else if(stack.getMetadata() == 5)
			{
				...
				tooltip.add(TextFormatting.GREEN + "- " + TextFormatting.BLUE + References.localNameVC("vc.item.tt.upgradeframe.#.3"));
			}
			else
			{
				//Make an airship move faster.
