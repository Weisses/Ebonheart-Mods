package com.viesis.viescraft.client.gui.airship.visual.core;
...
public class GuiVisualMenuCore extends GuiContainer {
	...
	public static int metaCore;
	private ResourceLocation texture = new ResourceLocation(References.MOD_ID + ":" + "textures/gui/container_airship_menu_visual_core.png");
	
	public GuiVisualMenuCore(IInventory playerInv, EntityAirshipBaseVC airshipIn)
	{
    ...
    public void initGui() 
    {
        ...
        if(this.airship.getMetaTierCore() > 1)
        {
        	GuiVC.buttonT2.visible = true;
        }
        if(this.airship.getMetaTierCore() > 2)
        {
        	GuiVC.buttonT3.visible = true;
        }
        if(this.airship.getMetaTierCore() > 3)
        {
        	GuiVC.buttonT4.visible = true;
        }
        if(this.airship.getMetaTierCore() > 4)
        {
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) 
	{
		...
		//if(this.airship.metaCoreVisualTransparent == true)
		//{
        ...
        //Disables the active button
		if(this.airship.metaCoreVisual == 0)
        {
        	GuiVC.buttonT1.enabled = false;
        }
        else 
        {
        	GuiVC.buttonT1.enabled = true;
        }
		if(this.airship.metaCoreVisual == 1)
        {
        	GuiVC.buttonT2.enabled = false;
        }
        else 
        {
        	GuiVC.buttonT2.enabled = true;
        }
		if(this.airship.metaCoreVisual == 2)
        {
        	GuiVC.buttonT3.enabled = false;
        }
        else 
        {
        	GuiVC.buttonT3.enabled = true;
        }
		if(this.airship.metaCoreVisual == 3)
        {
        	GuiVC.buttonT4.enabled = false;
        }
        else 
        {
        	GuiVC.buttonT4.enabled = true;
        }
		if(this.airship.metaCoreVisual == 4)
        {
        	GuiVC.buttonT5.enabled = false;
        }
        else 
        {
        	GuiVC.buttonT5.enabled = true;
        }
	}
	...
	{
		GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		this.fontRenderer.drawString(References.localNameVC("vc.main.models"), 73, 18, 11111111);
        ...
        if(this.airship.getMetaTierCore() > 0)
        {
        	this.drawItemStack(new ItemStack(InitItemsVC.UPGRADE_CORE, 1, 1), 7, 48, "");
        }
        if(this.airship.getMetaTierCore() > 1)
        {
        	this.drawItemStack(new ItemStack(InitItemsVC.UPGRADE_CORE, 1, 2), 7, 48 + (19 * 1), "");
        }
        if(this.airship.getMetaTierCore() > 2)
        {
        	this.drawItemStack(new ItemStack(InitItemsVC.UPGRADE_CORE, 1, 3), 7, 48 + (19 * 2), "");
        }
        if(this.airship.getMetaTierCore() > 3)
        {
        	this.drawItemStack(new ItemStack(InitItemsVC.UPGRADE_CORE, 1, 4), 7, 48 + (19 * 3), "");
        }
        if(this.airship.getMetaTierCore() > 4)
        {
        	this.drawItemStack(new ItemStack(InitItemsVC.UPGRADE_CORE, 1, 5), 7, 48 + (19 * 4), "");
        }
    }
