...
public class ItemUpgradeEngine extends Item {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
		...
		{
			tooltip.add(References.localNameVC("vc.item.tt.upgradeengine.0.1"));
			tooltip.add(References.localNameVC("vc.item.tt.upgradeengine.0.2"));
			tooltip.add("");
			tooltip.add(References.localNameVC("vc.item.tt.upgradeengine.0.3"));
		}
		else if(gameSettingsIn.isKeyDown(gameSettingsIn.keyBindSneak))
		{
			if(stack.getMetadata() == 1)
			{
				...
				tooltip.add(stringColorText + References.localNameVC("vc.item.tt.upgradeengine.#.1"));
				tooltip.add("");
				...
				//New engine skins each Tier.
				tooltip.add(TextFormatting.GREEN + "- " + TextFormatting.BLUE + References.localNameVC("vc.item.tt.upgradeengine.#.2"));
			}
			else if(stack.getMetadata() == 5)
			{
				...
				tooltip.add(TextFormatting.GREEN + "- " + TextFormatting.BLUE + References.localNameVC("vc.item.tt.upgradeengine.#.3"));
			}
			else
			{
				//Make an airship move faster.
	...
	public String getItemStackDisplayName(ItemStack stack)
    {
		...
		switch(stack.getMetadata())
		{
			...
			case 1:
				return colorName = TextFormatting.WHITE 
				+ References.localNameVC("item.vc:item_engine_upgrade.name") + " "
				+ TextFormatting.GRAY + "("
