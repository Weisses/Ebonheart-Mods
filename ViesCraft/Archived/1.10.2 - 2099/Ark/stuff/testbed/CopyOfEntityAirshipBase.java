...
import com.viesis.viescraft.common.entity.temp.EntityFuelVC;

public class CopyOfEntityAirshipBase extends EntityFuelVC implements IInventoryChangedListener
{
	
	private float AirshipSpeedTurn = 1.18F;
    ...
    //{
        ...
        //{
        //    return p_apply_1_ instanceof EntityAirshipBase && ((EntityAirshipBase)p_apply_1_).isBreeding();
        //}
    //};
    ...
    private static final DataParameter<Byte> STATUS = EntityDataManager.<Byte>createKey(CopyOfEntityAirshipBase.class, DataSerializers.BYTE);
    //private static final DataParameter<Integer> HORSE_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipBase.class, DataSerializers.VARINT);
    //private static final DataParameter<Integer> HORSE_VARIANT = EntityDataManager.<Integer>createKey(EntityAirshipBase.class, DataSerializers.VARINT);
    //private static final DataParameter<Optional<UUID>> OWNER_UNIQUE_ID = EntityDataManager.<Optional<UUID>>createKey(EntityAirshipBase.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Integer> HORSE_ARMOR = EntityDataManager.<Integer>createKey(CopyOfEntityAirshipBase.class, DataSerializers.VARINT);
    ...
    private AnimalChest airshipChest;
    ...
    public CopyOfEntityAirshipBase(World worldIn)
    {
        super(worldIn);
    ...
    public CopyOfEntityAirshipBase(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    {
        return "Airship";//this.hasCustomName() ? 
        		//this.getCustomNameTag() ;
        		//: this;//.getType().getDefaultName().getUnformattedText();
    }
    ...
    {
        return 
        		//!this.getType().isUndead() && 
        		super.canBeLeashedTo(player);
    }
    ...
    {
        return //this.canBeChested &&
        		//this.getType().canBeChested() && 
        		this.getHorseWatchableBoolean(8);
    }
    ...
    private int getChestSize()
    {
        ...
        return this.isChested() //&& horsetype.canBeChested() 
        		? 17 : 2;
    }

    private void initHorseChest()
    {
        AnimalChest animalchest = this.airshipChest;
        this.airshipChest = new AnimalChest("HorseChest", this.getChestSize());
        this.airshipChest.setCustomName(this.getName());

        if (animalchest != null)
        {
            animalchest.removeInventoryChangeListener(this);
            int i = Math.min(animalchest.getSizeInventory(), this.airshipChest.getSizeInventory());
            ...
            for (int j = 0; j < i; ++j)
            {
                ...
                if (itemstack != null)
                {
                    this.airshipChest.setInventorySlotContents(j, itemstack.copy());
                }
            }
        }

        this.airshipChest.addInventoryChangeListener(this);
        this.updateHorseSlots();
        this.itemHandler = new net.minecraftforge.items.wrapper.InvWrapper(this.airshipChest);
    }
    ...
    private void updateHorseSlots()
    {
        if (!this.worldObj.isRemote)
        {
            //this.setHorseSaddled(this.airshipChest.getStackInSlot(0) != null);

            //if (this.getType().isHorse())
            //{
                this.setHorseArmorStack(this.airshipChest.getStackInSlot(1));
            //}
        }
    }
...
/**    protected EntityAirshipBase getClosestHorse(Entity entityIn, double distance)
    {
        ...
        return (EntityAirshipBase)entity;
    }
...
*/
    ...
    {
        //this.openHorseMouth();
        return null;//this.getDeathSound();//.getType().getDeathSound();
    }
    ...
    protected SoundEvent getHurtSound()
    {
        ...
        return null;//this.getHurtSound();//.getType().getHurtSound();
    }
    ...
    protected SoundEvent getAmbientSound()
    {
        ...
        return null;//this.getAmbientSound();//.getType().getAmbientSound();
    }
    ...
    protected SoundEvent getAngrySound()
    {
        ...
        //this.makeHorseRear();
        //HorseType horsetype = this.getType();
        return null; //horsetype.isUndead() ? null : (horsetype.hasMuleEars() ? SoundEvents.ENTITY_DONKEY_ANGRY : SoundEvents.ENTITY_HORSE_ANGRY);
    }
...
*/
    public void openGUI(EntityPlayer playerEntity)
    {
        		...
        		)
        {
            ...
            playerEntity.openGui(ViesCraft.instance, GuiHandler.ENTITYAIRSHIPVC_GUI, getEntityWorld(), this.getPosition().getX(), this.getPosition().getY(), this.getPosition().getZ());
            
            //playerEntity.openGuiHorseInventory(this, this.airshipChest);
        }
    }
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand, @Nullable ItemStack stack)
    {
        ...
        else
        {
            if (stack != null)
            {
                ...
                if (!flag && 
                		//this.getType().canBeChested() && 
                		!this.isChested() && stack.getItem() == Item.getItemFromBlock(Blocks.CHEST))
                {
                ...
                if (!flag
                		//&& this.isRidable() 
                		&& !this.isHorseSaddled() && stack.getItem() == Items.SADDLE)
                {
                    this.openGUI(player);
                    return true;
                }
    ...
    public void onLivingUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            //{
            //    EntityAirshipBase testVC = this.getClosestHorse(this, 16.0D);

            //    if (testVC != null && this.getDistanceSqToEntity(testVC) > 4.0D)
            //    {
            //        this.navigator.getPathToEntityLiving(testVC);
            //    }
            //}
    ...
    {
        this.dropItemsInChest(this, this.airshipChest);
        this.dropChests();
    }
...
*/
    ...
    {
        EntityLiving.func_189752_a(p_189803_0_, "testVC");
        p_189803_0_.registerWalker(FixTypes.ENTITY, new ItemStackDataLists("testVC", new String[] {"Items"}));
        p_189803_0_.registerWalker(FixTypes.ENTITY, new ItemStackData("testVC", new String[] {"ArmorItem", "SaddleItem"}));
    }
    ...
    public void writeEntityToNBT(NBTTagCompound compound)
    {
        ...
        if (this.isChested())
        {
            ...
            for (int i = 2; i < this.airshipChest.getSizeInventory(); ++i)
            {
                ItemStack itemstack = this.airshipChest.getStackInSlot(i);
        ...
        if (this.airshipChest.getStackInSlot(1) != null)
        {
            compound.setTag("ArmorItem", this.airshipChest.getStackInSlot(1).writeToNBT(new NBTTagCompound()));
        }

        if (this.airshipChest.getStackInSlot(0) != null)
        {
            compound.setTag("SaddleItem", this.airshipChest.getStackInSlot(0).writeToNBT(new NBTTagCompound()));
        }
    }
    ...
    public void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        if (this.isChested())
        {
            ...
            for (int i = 0; i < nbttaglist.tagCount(); ++i)
            {
                ...
                if (j >= 2 && j < this.airshipChest.getSizeInventory())
                {
                    this.airshipChest.setInventorySlotContents(j, ItemStack.loadItemStackFromNBT(nbttagcompound));
                }
            }
        }
        ...
        if (compound.hasKey("ArmorItem", 10))
        {
            ...
            {
                this.airshipChest.setInventorySlotContents(1, itemstack);
            }
        }
        ...
        if (compound.hasKey("SaddleItem", 10))
        {
            ...
            {
                this.airshipChest.setInventorySlotContents(0, itemstack1);
            }
        }
	...
	//	return allowStandSliding;
        ...
        //else
        //{
        //    testVC testVC = (testVC)otherAnimal;

            //if (this.canMate() && testVC.canMate())
            //{
            //    HorseType horsetype = this.getType();
            //    HorseType horsetype1 = testVC.getType();
    ...
    //{
	//	return null;
        //testVC testVC = (testVC)ageable;
     //   testVC testVC1 = new testVC(this.worldObj);
     //   HorseType horsetype = this.getType();
        //HorseType horsetype1 = testVC.getType();
...
/**
        ...
        if (horsetype2 == HorseType.HORSE)
        {
            ...
            {
                i = i | testVC.getHorseVariant() & 65280;
            }
            else
            {
            ...
            testVC1.setHorseVariant(i);
        }

        testVC1.setType(horsetype2);
        ...
        testVC1.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(d1 / 3.0D);
        ...
        testVC1.getEntityAttribute(JUMP_STRENGTH).setBaseValue(d2 / 3.0D);
        ...
        testVC1.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(d0 / 3.0D);
        return testVC1;
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata)
    {
        ...
        //if (livingdata instanceof EntityAirshipBase.GroupData)
        //{
        //    horsetype = ((EntityAirshipBase.GroupData)livingdata).horseType;
        //    i = ((EntityAirshipBase.GroupData)livingdata).horseVariant & 255 | this.rand.nextInt(5) << 8;
        //}
        //else
        //{
        ...
        //    }
            ...
            //livingdata = new EntityAirshipBase.GroupData(horsetype, i);
        //}
...
*/
    ...
    public boolean replaceItemInInventory(int inventorySlot, @Nullable ItemStack itemStackIn)
    {
        ...
        if (i >= 0 && i < 2 && i < this.airshipChest.getSizeInventory())
        {
            ...
            {
                this.airshipChest.setInventorySlotContents(i, itemStackIn);
                this.updateHorseSlots();
                return true;
            }
            else
        ...
        else
        {
            ...
            if (j >= 2 && j < this.airshipChest.getSizeInventory())
            {
                this.airshipChest.setInventorySlotContents(j, itemStackIn);
                return true;
            }
            else
            {
