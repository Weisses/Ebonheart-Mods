...
public class MessageGuiV2ModuleInventoryLarge extends MessageBase<MessageGuiV2ModuleInventoryLarge> {
	...
	{
		player.openGui(ViesCraft.instance, GuiHandler.GUI_V2_MODULE_INVENTORY_LARGE, player.world, player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ());
	}
}
