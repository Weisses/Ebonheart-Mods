...
public class EntityAirshipRed extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipRed.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipRed.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipRed.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipRed.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipRed.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipRed.Status status;
    public EntityAirshipRed.Status previousStatus;
    ...
    public EntityAirshipRed(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipRed.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_red;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipRed.Status.UNDER_WATER && this.status != EntityAirshipRed.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipRed.Status getAirshipStatus()
    {
        EntityAirshipRed.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipRed.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipRed.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipRed.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipRed.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipRed.Status EntityAirshipEA$status = EntityAirshipRed.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipRed.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipRed.Status.IN_AIR && this.status != EntityAirshipRed.Status.IN_AIR && this.status != EntityAirshipRed.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipRed.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipRed.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipRed.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipRed.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipRed.Status.IN_AIR
            	  || this.status == EntityAirshipRed.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipRed.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipRed.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipRed.Type getBoatType()
    {
        return EntityAirshipRed.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipRed.Type byId(int id)
        {
        ...
        public static EntityAirshipRed.Type getTypeFromString(String nameIn)
        {
