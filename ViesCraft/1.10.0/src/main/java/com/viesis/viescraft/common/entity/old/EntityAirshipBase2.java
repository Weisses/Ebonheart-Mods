...
public class EntityAirshipBase2 extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipBase2.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipBase2.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipBase2.class, DataSerializers.FLOAT);
	...
	public float bladespeed = 3.0F;
	
	
	public EntityAirshipBase2.Status status;
    public EntityAirshipBase2.Status previousStatus;
    
    private float AirshipSpeedTurn = 0.09F;//0.18F;
    private float AirshipSpeedForward = 0.01875F;//0.0125F;
    private float AirshipSpeedUp = 0.00525F;//0.0035F;
    private float AirshipSpeedDown = 0.00525F;//0.0035F;
    
    public EntityPlayer player = Minecraft.getMinecraft().thePlayer;
    
	public EntityAirshipBase2(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipBase2(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    {
        return InitItemsVC.item_airship_v2;
    }
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipBase2.Status.UNDER_WATER && this.status != EntityAirshipBase2.Status.UNDER_FLOWING_WATER)
        {
        ...
        if (this.canPassengerSteer())
        {
            ...
            if (this.worldObj.isRemote)
            {
                ...
                //}
            	//LogHelper.info(this.getEntityId() + "     " + player.getRidingEntity().getEntityId());
                
            	//if(this.motionX != 0
            	//|| this.motionY != 0
            	//|| this.motionZ != 0
                		//&& player.getRidingEntity() != null
                		//|| player.getRidingEntity().getEntityId() != null
                //		)
                //{
                //	this.bladespeed = 3.0F;
                //}
                //else
                //{
     ...
     */
    private EntityAirshipBase2.Status getAirshipStatus()
    {
        EntityAirshipBase2.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipBase2.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipBase2.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipBase2.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipBase2.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipBase2.Status EntityAirshipEA$status = EntityAirshipBase2.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipBase2.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipBase2.Status.IN_AIR && this.status != EntityAirshipBase2.Status.IN_AIR && this.status != EntityAirshipBase2.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipBase2.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipBase2.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipBase2.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipBase2.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipBase2.Status.IN_AIR
            	  || this.status == EntityAirshipBase2.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        //{
            //this.setBoatType(EntityAirshipBase.Type.getTypeFromString(compound.getString("Type")));
        //}
    }
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityAirshipBase.Status.ON_LAND)
                    {
    ...
    public void setBoatType(EntityAirshipBase2.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipBase2.Type getBoatType()
    {
        return EntityAirshipBase2.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipBase2.Type byId(int id)
        {
        ...
        public static EntityAirshipBase2.Type getTypeFromString(String nameIn)
        {
