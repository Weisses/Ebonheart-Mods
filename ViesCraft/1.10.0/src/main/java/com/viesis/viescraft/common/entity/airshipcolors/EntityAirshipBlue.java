...
public class EntityAirshipBlue extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipBlue.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipBlue.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipBlue.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipBlue.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipBlue.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipBlue.Status status;
    public EntityAirshipBlue.Status previousStatus;
	...
	public EntityAirshipBlue(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipBlue(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipBlue.Type.NORMAL.ordinal()));
	}
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipBlue.Status.UNDER_WATER && this.status != EntityAirshipBlue.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipBlue.Status getAirshipStatus()
    {
        EntityAirshipBlue.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipBlue.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipBlue.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipBlue.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipBlue.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipBlue.Status EntityAirshipEA$status = EntityAirshipBlue.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipBlue.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipBlue.Status.IN_AIR && this.status != EntityAirshipBlue.Status.IN_AIR && this.status != EntityAirshipBlue.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipBlue.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipBlue.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipBlue.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipBlue.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipBlue.Status.IN_AIR
            	  || this.status == EntityAirshipBlue.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipBlue.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipBlue.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipBlue.Type getBoatType()
    {
        return EntityAirshipBlue.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipBlue.Type byId(int id)
        {
        ...
        public static EntityAirshipBlue.Type getTypeFromString(String nameIn)
        {
