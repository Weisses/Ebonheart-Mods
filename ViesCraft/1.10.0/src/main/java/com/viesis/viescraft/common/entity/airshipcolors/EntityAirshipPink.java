...
public class EntityAirshipPink extends EntityVC {
	
	private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityAirshipPink.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityAirshipPink.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityAirshipPink.class, DataSerializers.FLOAT);
	private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityAirshipPink.class, DataSerializers.VARINT);
    
	private static final DataParameter<Boolean> POWERED = EntityDataManager.<Boolean>createKey(EntityAirshipPink.class, DataSerializers.BOOLEAN);
    private int fuel;
	
	public EntityAirshipPink.Status status;
    public EntityAirshipPink.Status previousStatus;
	...
	public EntityAirshipPink(World worldIn)
    {
        super(worldIn);
    ...
    public EntityAirshipPink(World worldIn, double x, double y, double z)
    {
        this(worldIn);
	...
	protected void entityInit() 
	{
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityAirshipPink.Type.NORMAL.ordinal()));
	}
    ...
    {
		return InitItemsVC.item_airship_pink;
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityAirshipPink.Status.UNDER_WATER && this.status != EntityAirshipPink.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityAirshipPink.Status getAirshipStatus()
    {
        EntityAirshipPink.Status EntityAirshipEA$status = this.getUnderwaterStatus();
        ...
        {
            return EntityAirshipPink.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityAirshipPink.Status.ON_LAND;
            }
            else
            {
                return EntityAirshipPink.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityAirshipPink.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityAirshipPink.Status EntityAirshipEA$status = EntityAirshipPink.Status.UNDER_FLOWING_WATER;
                                return EntityAirshipEA$status;
                            }
        ...
        return flag ? EntityAirshipPink.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityAirshipPink.Status.IN_AIR && this.status != EntityAirshipPink.Status.IN_AIR && this.status != EntityAirshipPink.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityAirshipPink.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityAirshipPink.Status.IN_WATER)
            {
            	this.momentum = 0.45F;
            }
            else if (this.status == EntityAirshipPink.Status.UNDER_FLOWING_WATER 
        	  || this.status == EntityAirshipPink.Status.UNDER_WATER)
            {
            	if (!this.worldObj.isRemote)
            	{
            ...
            else if (this.status == EntityAirshipPink.Status.IN_AIR
            	  || this.status == EntityAirshipPink.Status.ON_LAND)
            {
            	this.momentum = 0.9F;
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound)
    {
        ...
        {
            this.setBoatType(EntityAirshipPink.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntityAirshipPink.Type boatType)
    {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntityAirshipPink.Type getBoatType()
    {
        return EntityAirshipPink.Type.byId(((Integer)this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntityAirshipPink.Type byId(int id)
        {
        ...
        public static EntityAirshipPink.Type getTypeFromString(String nameIn)
        {
